...
1. 官方二进制安装
2. 官方kubeadm 安装  Docker 部署
3. 第三方工具 ： 
    * sealos （很有意思，快速部署，安装简单）
    * rancher (比较完善，有自己的部署界面，国人开发，集群部署相对步骤较多)
    * kubespray 基于Ansible的部署方案，部署依赖于ansible
    * minikube 在本地快速运行一个单点的Kubernetes，尝试Kubernetes或日常开发的用户使用。不能用于生产环境。
...
生成证书：
cfssl gencert -initca ca-csr.jsion | cfssljson -bare ca 
cfssl gencert -ca=ca.pem -ca-key=ca-key.pem -config=ca-config.json -profile=www sercer-csr.json | cfssljson -bare server
# ls *pem
...
member a342ea2798d20705 is healthy: got healthyresult from https://192.168.31.66:2379 
cluster is healthy
...
以上部署步骤在规划的每个node节点都操作
下载二进制包：
# wget https://github.com/coreos/flannel/releases/download/v0.10.0/flannel-v0.10.0-linux-amd64.tar.gz
# tar zxvf flannel-v0.9.1-linux-amd64.tar.gz
# mv flanneld mk-docker-opts.sh /opt/kubernetes/bin
配置Flannel：
# cat /opt/kubernetes/cfg/flanneld FLANNEL_OPTIONS="--etcd-endpoints=https://192.168.31.63:2379,https://192.168.31.65:2379,https://192.168.31.66:2 379 -etcd-cafile=/opt/etcd/ssl/ca.pem -etcd-certfile=/opt/etcd/ssl/server.pem -etcdkeyfile=/opt/etcd/ssl/server-key.pem" 
systemd管理Flannel：
...
--service-node-port-range Service Node类型默认分配端口范围 systemd管理
apiserver：
...
--leader-elect 当该组件启动多个时，自动选举（HA） systemd管理schduler
...
所有组件都已经启动成功，通过kubectl工具查看当前群组件状态：
...
如上输出说明组件正常
5 在NODE节点部署组件
Master apiserver启用TLS认证后，Node节点kubelet组件想要加入集群，必须使用CA签发的有效证书才能与 apiserver通信，当Node节点很多时，签署证书是一件很繁琐的事情，因此有了TLS Bootstrapping机制，kubelet 会以一个低权限用户自动向apiserver申请证书，kubelet的证书由apiserver动态签署。认证大致工作流程如图所示：
...
--cert-dir 颁发证书存放位置
--pod-infra-container-image 管理Pod网络的镜像其中/opt/kubernetes/cfg/kubelet.config配置文件如下：
...
# kubectl get node
5.3 部署kube-proxy组件     创建kube-proxy配置文件：
